...
export function PromptPreview({
  ...
  const downloadPrompt = () => {
    ...
    anchor.href = url;
    anchor.download = `${safeFileName(projectName)}-implementation-prompt.md`;
  ...
  return (
    <div className="spec-preview-backdrop" onClick={onClose}>
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label="実装Promptプレビュー"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="spec-preview__grabber" aria-hidden="true" />
        <header className="spec-preview__header">
          <div>
            <div className="spec-preview__eyebrow">Prompt Generator</div>
            <h2>実装Prompt</h2>
          </div>
          <button type="button" className="icon-button" onClick={onClose} aria-label="Promptを閉じる">
            ×
          </button>
        </header>
        ...
        <div className="spec-preview__status">
          ...
          <span>
            {errorCount > 0
              ? '未設定値を勝手に補完しない指示をPromptへ含めています'
              : 'CodexなどのCoding LLMへそのまま渡せます'}
          </span>
        </div>
        ...
        <footer className="spec-preview__footer">
          ...
          <div className="spec-preview__actions">
            <button type="button" className="project-action" onClick={downloadPrompt}>
              .md保存
            </button>
            <button type="button" className="spec-primary-button" onClick={() => void copyPrompt()}>
              コピー
            </button>
          </div>
        </footer>
