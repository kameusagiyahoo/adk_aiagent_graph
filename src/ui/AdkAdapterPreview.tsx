...
import type { AdkAdapterAnalysis } from '../adapters/adk/types';
...
export function AdkAdapterPreview({ projectName, analysis, onClose }: AdkAdapterPreviewProps) {
  ...
  const copyAnalysis = async () => {
    try {
      ...
      } else if (!fallbackCopy(analysis.markdown)) {
  ...
  return (
    <div className="spec-preview-backdrop" onClick={onClose}>
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Google ADK Adapter readiness"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="spec-preview__grabber" aria-hidden="true" />
        <header className="spec-preview__header">
          <div>
            <div className="spec-preview__eyebrow">Google ADK Adapter</div>
            <h2>ADK変換チェック</h2>
          </div>
          <button type="button" className="icon-button" onClick={onClose} aria-label="ADK変換チェックを閉じる">
            ×
          </button>
        </header>
        ...
        <div className="spec-preview__status">
          ...
          <span className={`spec-status ${analysis.partialCount > 0 ? 'spec-status--warning' : ''}`}>
            Partial {analysis.partialCount}
          </span>
          <span className={`spec-status ${analysis.blockedCount > 0 ? 'spec-status--error' : ''}`}>
        ...
        <footer className="spec-preview__footer">
          ...
          <div className="spec-preview__actions">
            <button type="button" className="project-action" onClick={downloadAnalysis}>
              .md保存
            </button>
            <button type="button" className="spec-primary-button" onClick={() => void copyAnalysis()}>
              コピー
            </button>
          </div>
        </footer>
